...
FMT_AGE = "<B6s4I12?32sHH"   # + left/right ms since the hit (newer firmware)
...
while n < 8 and time.time() - start < 15:
    try:
        ...
        break
    n += 1
    if len(data) == struct.calcsize(FMT_AGE):
        f = struct.unpack(FMT_AGE, data)
        age = f"  hit_age L{f[19]}ms R{f[20]}ms"
    elif len(data) == struct.calcsize(FMT):
